...
def pooling_layer_forward(input, layer):
    ...
    y = np.zeros((h_out, w_out, c, batch_size))
    X = input['data'].reshape(h_in, w_in, c, batch_size, order="F")
    for b in range(batch_size):
        for ch in range(c):
            for i in range(h_out):
                for j in range(w_out):
                    y[i, j, ch, b] = np.max(X[i*stride:i*stride+k, j*stride:j*stride+k, ch, b])
    #print (y)
    ...
    output['data'] = y.reshape(-1, y.shape[-1], order="F") # replace with your implementation
