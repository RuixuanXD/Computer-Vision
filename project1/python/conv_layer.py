...
def conv_layer_forward(input_data, layer, param):
    ...
    W = param['w']
    b = param['b']
    # y = np.zeros((h_out, w_out, num, batch_size))
    # X = input_data['data'].reshape(h_in, w_in, c, batch_size)
    # X = np.pad(X, ((pad, pad), (pad, pad), (0, 0), (0, 0)), mode='constant')
    # input_n['data'] = X
    # input_col = im2col_conv_batch(input_n, layer, h_out, w_out)

    X = im2col_conv_batch(input_n, layer, h_out, w_out) #3D
    y = np.zeros((num, h_out * w_out, batch_size))
    
    for i in range(batch_size):
        y[:,:,i] = np.matmul(W.T,X[:,:,i])
    for i in range(batch_size):
        y[:,:,i] = (y[:,:,i].T + b).T
    # output_data = np.dot(X, W)
    # output_data = auto_add(output_data, b)

    y = y.reshape((num*h_out*w_out, batch_size))
    #print(output)
    #print(y)
    ...
    # for n in range(num):
    #     for ch in range(c):
    #         y[n] += np.dot(W[ch*k*k:(ch+1)*k*k, n], X[ch*k*k:(ch+1)*k*k])
...
#         if A.shape[-i] != b.shape[-i] and b.shape[-i] == 1:
#             continue
#         elif A.shape[-i] != b.shape[-i]:
#             b = np.moveaxis(b, -i, 0)
